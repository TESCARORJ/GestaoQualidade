    ...
    public class ProdutividadeMaoObraAppService : IProdutividadeMaoObraAppService
    {
        ...
        public async Task GararPerguntasAsync()
        {
            ...
            using (var transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                ...
                var log = new LogCrud()
                {
                    ...
                    Descricao = $"Cadastrado períodos de preenchimento de Produtividade de Mão de Obra para o usuário {usuarioLogado.Nome} em {DateTime.Now}",
                };
        ...
        public async Task<ProdutividadeMaoObraResponseDTO> DeleteAsync(long id)
        {
            ...
            var logCrud = new LogCrud
            {
                ...
                Descricao = $" Produtividade de Mão de Obra do período {model.Mes} excluída no sistema por {usuarioLogado.Nome}, ID: {usuarioLogado.Id} em {DateTime.Now}."
            };
        ...
        private async Task<List<LogCrud>> Logar(ProdutividadeMaoObraRequestDTO request, string tabela)
        {
            var logs = new List<LogCrud>();
            var usuarioLogado = _usuarioSession.GetUsuarioLogado().Result;
