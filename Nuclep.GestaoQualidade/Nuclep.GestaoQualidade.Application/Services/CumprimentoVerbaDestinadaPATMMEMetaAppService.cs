    ...
    public class CumprimentoVerbaDestinadaPATMMEMetaAppService : ICumprimentoVerbaDestinadaPATMMEMetaAppService
    {
        ...
        public async Task<CumprimentoVerbaDestinadaPATMMEMetaResponseDTO> DeleteAsync(long id)
        {
            ...
            var logCrud = new LogCrud
            {
                ...
                LogTabelaId = _logTabelaRepository.GetOneAsync(x => x.Nome.ToLower() == "Ind_CumprimentoVerbaDestinadaPATMMEMeta".ToLower()).Result.Id,
                IdReferencia = model.Id,
                Descricao = $"Meta Cumprimento Verba Destinada PAT MME do período {model.Meta} excluída no sistema por {usuarioLogado.Nome}, ID: {usuarioLogado.Id} em {DateTime.Now}."
            };
        ...
        private async Task<List<LogCrud>> Logar(CumprimentoVerbaDestinadaPATMMEMetaRequestDTO request, string tabela)
        {
