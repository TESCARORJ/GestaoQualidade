    ...
    public class CumprimentoVerbaDestinadaPATMMEAppService : ICumprimentoVerbaDestinadaPATMMEAppService
    {
        ...
        public async Task GararPerguntasAsync()
        {
            ...
            using (var transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                ...
                var log = new LogCrud()
                {
                    ...
                    Descricao = $"Cadastrado períodos de preenchimento de Cumprimento Verba Destinada PAT MME para o usuário {usuarioLogado.Nome} em {DateTime.Now}",
                };
        ...
        public async Task<CumprimentoVerbaDestinadaPATMMEResponseDTO> DeleteAsync(long id)
        {
            ...
            var logCrud = new LogCrud
            {
                ...
                UsuarioNome = usuarioLogado.Nome,
                LogTipo = LogTipo.Cadastrado,
                LogTabelaId = _logTabelaRepository.GetOneAsync(x => x.Nome.ToLower() == "Ind_CumprimentoVerbaDestinadaPATMME".ToLower()).Result.Id,
                IdReferencia = model.Id,
                Descricao = $" Cumprimento Verba Destinada PAT MME do período {model.Semestre} excluída no sistema por {usuarioLogado.Nome}, ID: {usuarioLogado.Id} em {DateTime.Now}."
            };
        ...
        private async Task<List<LogCrud>> Logar(CumprimentoVerbaDestinadaPATMMERequestDTO request, string tabela)
        {
            var logs = new List<LogCrud>();
            var usuarioLogado = _usuarioSession.GetUsuarioLogado().Result;
