    ...
    public class DuracaoProcessoLicitacaoAppService : IDuracaoProcessoLicitacaoAppService
    {
        ...
        public async Task GararPerguntasAsync()
        {
            ...
            using (var transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                ...
                var log = new LogCrud()
                {
                    ...
                    Descricao = $"Cadastrado períodos de preenchimento de Duração Processo Licitação para o usuário {usuarioLogado.Nome} em {DateTime.Now}",
                };
        ...
        public async Task<DuracaoProcessoLicitacaoResponseDTO> DeleteAsync(long id)
        {
            ...
            var logCrud = new LogCrud
            {
                ...
                Descricao = $" Duração Processo Licitação do período {model.Trimestre} excluída no sistema por {usuarioLogado.Nome}, ID: {usuarioLogado.Id} em {DateTime.Now}."
            };
        ...
        private async Task<List<LogCrud>> Logar(DuracaoProcessoLicitacaoRequestDTO request, string tabela)
        {
            var logs = new List<LogCrud>();
            var usuarioLogado = _usuarioSession.GetUsuarioLogado().Result;
